...
def split_df(df, key):
    ...
    :param df: Molecular structure(s) in lba dataframe format.
...
def bp_to_df(bp):
    """Convert biopython representation to lba dataframe representation.

    :param bp: Molecular structure in Biopython representation.
    :type bp: Bio.PDB.Structure

    :return: Molecular structure in lba dataframe format.
...
def df_to_bp(df_in):
    """Convert lba dataframe representation to biopython representation. Assumes dataframe contains only one structure.

    :param df_in: Molecular structure in lba dataframe format.
...
def df_to_bps(df_in):
    """Convert lba dataframe representation containing multiple structures to list of Biopython structures. Assumes different structures are specified by `ensemble` and `structure` columns of dataframe.

    :param df_in: Molecular structures in lba dataframe format.
    :type df_in: pandas.DataFrame
...
def read_any(f, name=None):
    """Read any lba file type into Biopython structure (compatible with pdb, pdb.gz, mmcif, sdf, xyz).
...
def mol_to_df(mol, add_hs=False, structure=None, model=None, ensemble=None, residue=999):
    """
    Convert molecule in RDKit format to lba dataframe format, with PDB-style columns.

    :param mol: Molecule in RDKit format.
    :type mol: rdkit.Chem.rdchem.Mol

    :return: Dataframe in standard lba format.
    :rtype: pandas.DataFrame
    """
