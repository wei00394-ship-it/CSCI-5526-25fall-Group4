...
class CNN3D_TransformLBA(object):
    ...
    def __call__(self, item):
        # Convert atoms dicts back to DataFrames if they are dicts
        try:
            if isinstance(item['atoms_pocket'], dict):
                if 'columns' in item['atoms_pocket'] and 'data' in item['atoms_pocket']:
                    item['atoms_pocket'] = pd.DataFrame(
                        data=item['atoms_pocket']['data'],
                        columns=item['atoms_pocket']['columns'],
                        index=item['atoms_pocket']['index'] if 'index' in item['atoms_pocket'] else None
                    )
                else:
                    item['atoms_pocket'] = pd.DataFrame(item['atoms_pocket'])
            
            if isinstance(item['atoms_ligand'], dict):
                if 'columns' in item['atoms_ligand'] and 'data' in item['atoms_ligand']:
                    item['atoms_ligand'] = pd.DataFrame(
                        data=item['atoms_ligand']['data'],
                        columns=item['atoms_ligand']['columns'],
                        index=item['atoms_ligand']['index'] if 'index' in item['atoms_ligand'] else None
                    )
                else:
                    item['atoms_ligand'] = pd.DataFrame(item['atoms_ligand'])
        except ValueError as e:
            print(f"Error converting to DataFrame for item {item.get('id', 'unknown')}")
...
if __name__=="__main__":
    ...
    dataset = LMDBDataset(dataset_path, transform=CNN3D_TransformLBA(radius=20.0))
    dataloader = DataLoader(dataset, batch_size=144, shuffle=False)
