...
import lba.splits.splits as spl
...
class Scores(object):
    ...
    def __init__(self, score_path):
        self._scores = pd.read_csv(
            ...
            index_col='pdb_id', usecols=['pdb_id', 'pKd']).dropna()
        # Rename columns to match internal usage if necessary, or just rely on index
        # The code uses self._scores.loc[pdbcode], which returns a Series (if 1 col) or DataFrame/Series
        # self._scores.loc[pdbcode].to_dict() will keys as column names.
        # Original code expected 'neglog_aff'.
        # Let's rename 'pKd' to 'neglog_aff' to keep compatibility with downstream code if it uses that key.
        self._scores.rename(columns={'pKd': 'neglog_aff'}, inplace=True)
        # Ensure index is string and lowercased for robust lookup
        self._scores.index = self._scores.index.astype(str).str.lower()

    def _lookup(self, pdbcode):
        # pdbcode should also be lowercased
        pdbcode = str(pdbcode).lower()
...
class LBADataset(Dataset):
    ...
    def _load_datasets(self, input_file_path, pdbcodes, ligcodes):
        ...
        for ligcode in ligcodes:
            # ligcode is a Path object from find_files
            filename = ligcode.name
            # filename is like "1arj_ligand.sdf"
            pdbcode = filename[:4]
        ...
        logger.info(f'Found {len(ligand_list):} protein/ligand files...')
        ...
        self._pocket_dataset = da.load_dataset(pocket_list, 'pdb',
                                               transform=None)
        self._ligand_dataset = da.load_dataset(ligand_list, 'sdf', add_hs=False,
...
def split_lmdb_dataset(lmdb_path, train_txt, val_txt, test_txt, split_dir):
    ...
    def _write_split_indices(split_txt, lmdb_ds, output_txt):
        with open(split_txt, 'r') as f:
            split_set = set([x.strip().lower() for x in f.readlines()]) # Normalize to lowercase
        
        # Get all IDs from LMDB, also normalize to lowercase for comparison if needed
        # But we must match the exact ID stored in LMDB to get the correct index.
        # Assuming lmdb_ds.ids() returns the IDs as stored (which were lowercased in process_pdbbind)
        all_ids = lmdb_ds.ids()
        
        # Filter IDs that are in the split set
        split_ids = list(filter(lambda id: id.lower() in split_set, all_ids))
...
def make_lmdb_dataset(input_file_path, score_path, output_root):
    ...
    pdbcodes = fi.find_files(input_file_path, 'cif')
    ligcodes = fi.find_files(input_file_path, 'sdf')
    #print(ligcodes)

    # Filter ligcodes based on score availability
    if scores:
        valid_ligcodes = []
        for lig in ligcodes:
            # Assuming format like '1arj_ligand.sdf' -> '1arj'
            # Adjust if filename format differs, but this matches LBADataset assumption
            pdbcode = lig.name[:4] 
            if scores._lookup(pdbcode) is not None:
                valid_ligcodes.append(lig)
            else:
                logger.warning(f"Skipping {pdbcode}: No score found in {score_path}")
        ligcodes = valid_ligcodes
        logger.info(f"Filtered to {len(ligcodes)} ligands with scores.")
...
@click.argument('input_file_path', type=click.Path()) #where the clean data produced by process_pdbbind.py is.
@click.argument('output_root', type=click.Path()) #the path to store .mdb data
@click.option('--no_gen', '-ng', is_flag=True,
              help="If specified, we don't process the data to lmdb")
@click.option('--split', '-s', is_flag=True)
@click.option('--train_txt', '-tr', type=click.Path(exists=True), default=None) #train_list
@click.option('--val_txt', '-v', type=click.Path(exists=True), default=None) #val_list
@click.option('--test_txt', '-t', type=click.Path(exists=True), default=None) #test_list
@click.option('--score_path', type=click.Path(exists=True), default=None) #where the pdbbind_NL_cleaned.csv is.
